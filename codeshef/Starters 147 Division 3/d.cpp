...
void Akshay() {
    ...
    // Traverse each element and check pair sums using a hashmap
    for (int i = 0; i < n; ++i) {
        // Check if there is a valid pair involving the current element
        for (const auto& pair : valueToIndex) {
            int sum = pair.first + v[i];
            if (!isprime(sum)) {
                cout << pair.second + 1 << " " << i + 1 << endl;
                return;
            }
        }
