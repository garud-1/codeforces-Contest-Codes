...
void solve(){
    ll n;
    cin >> n;
    ll ans = 2; // Initialize ans with 2
    ll maxi = INT_MIN; // Initialize maxi with minimum value

    for(ll i = 2; i <= n; i++){
        ll k = n / i; // find the maximum k such that k * i <= n
        ll sum = k * i; // calculate the sum of multiples of i
        if(sum > maxi){
            maxi = sum;
            ans = i; // Update ans if sum is greater than previous maximum
        }
    }
